...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	vector< vector <float> > newGrid (grid.size(),vector<float>());
        ...
        for(int i=0;i < grid.size();i++)
         num_elements += grid[i].size();
	// your code here
        float uni_prob = 1.0 / num_elements;
        for(int i=0;i < newGrid.size();i++) {
         vector<float> temp_vector;
         for(int j=0;j < grid[i].size();j++) 
          temp_vector.push_back(uni_prob);
	...
	float p_miss) 
{
	...
	// your code here
        for(int i=0;i < grid.size();i++) {
         ...
         for(int j=0;j < grid[i].size();j++) {
          if(color == grid[i][j])
           temp_vector.push_back(beliefs[i][j] * p_hit);
          else
           temp_vector.push_back(beliefs[i][j] * p_miss);
         }
         newGrid[i] = temp_vector;
        }
	...
	float blurring) 
{
	...
	vector < vector <float> > newGrid (beliefs.size(),vector<float>());
	...
	// your code here
        for(int i=0;i < beliefs.size();i++) {
         for(int j=0;j < beliefs[i].size();j++) {
          int new_x = (i + dy) % beliefs.size();
          int new_y = (j + dx) % beliefs[new_x].size();
          newGrid[new_x][new_y] = beliefs[i][j];
         }
        }
